...
uses
  ...
  MPEGPlayer in 'MPEGPlayer.pas',
  Player in 'Player.pas';

var
  Play: TPlayer;
begin
  Play := TMPEGPlayer.Create;
  Play.LoadFile('J:\Music\kosheen - catch.mp3');
//  Play.LoadFile('M:\Misc Music\KoRn\Korn - Good God (Rammstein Mix).mp3');
  Play.SetOutput(CreateMCIOBffer(Play));
  Play.DoRepeat := True;
  Play.Play;
  try
    while Play.IsPlaying do begin
      Write(Format(#13'Playing: %d:%.2d of %d:%.2d (%d KBps)', [Play.Position div 60, Play.Position mod 60, Play.Length div 60, Play.Length mod 60, Play.Bitrate div 1000]));
      Sleep(50);

      if (GetKeyState(VK_ESCAPE) and $8000 <> 0) then begin
        Play.Stop;
        break;
      end;
    end;
