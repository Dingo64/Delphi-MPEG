...
begin
  ...
  if (h.val = nil) then begin
    Result := 2;
    exit;
  end;

  // table 0 needs no bits
  if (h.treelen = 0) then begin
    x := 0;
    y := 0;
    Result := 0;
    exit;
  ...
  repeat
    if (h.val[point, 0] = 0) then begin  // end of tree
  ...
  if (h.tablename[0] = '3') and (
     (h.tablename[1] = '2') or (h.tablename[1] = '3')) then begin
    v := (y shr 3) and 1;
    w := (y shr 2) and 1;
    x := (y shr 1) and 1;
    y := y and 1;
