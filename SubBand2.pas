...
interface
uses
  Windows, SysUtils, Shared, BitStream, SubBand, Header, CRC, SynthFilter;
...
begin
  if (Header.Version = MPEG1) then begin
  ...
  end else begin  // MPEG-2 LSF -- Jeff
...
begin
  ...
  if (ChannelBitrate = 1) or (ChannelBitrate = 2) then begin
    ...
    // tables 3-B.2a or 3-B.2b
    if (FSubBandNumber <= 2) then begin
...
begin
  if (FAllocation <> 0) then
    if (FGroupingTable <> nil) then begin
...
begin
  if (FAllocation <> 0) then begin
    ...
    if (Channels = Both) then begin
      Sample2 := Sample;
      if (FGroupNumber <= 4) then begin
        Sample := Sample * FScaleFactor1;
	Sample2 := Sample2 * FChannel2ScaleFactor1;
      end else if (FGroupNumber <= 8) then begin
        Sample := Sample * FScaleFactor2;
...
begin
  ...
  if (FChannel2Allocation <> 0) then
    if (FChannel2GroupingTable <> nil) then begin
