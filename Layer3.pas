...
interface
uses
  Windows, Shared, BitReserve, BitStream, Header, OBuffer, SynthFilter, L3Type;
...
begin
  ...
  FSI := AllocMem(Sizeof(TIIISideInfo));
end;
  ...
  for gr := 0 to FMaxGr-1 do begin
    ...
    for ch := FFirstChannel to FLastChannel do begin
      ...
      if ((ch = 0) or (FWhichChannels = Right)) then begin
        for ss := 0 to SSLIMIT-1 do begin  // Polyphase synthesis
          sb := 0;
          sb18 := 0;
...
var new_slen: array[0..3] of Cardinal;
...
begin
  ...
  if ((((mode_ext = 1) or (mode_ext = 3)) and (ch = 1))) then begin
  ...
  m := 0;
  for i := 0 to 3 do
    for j := 0 to NrOfSFBBlock[blocknumber, blocktypenumber, i] do begin
...
begin
  ...
  if ((gr_info.window_switching_flag <> 0) and (gr_info.block_type = 2)) then begin 
    ...
    if ((FSI.ch[ch].scfsi[0] = 0) or (gr = 0)) then begin
    ...
    if ((FSI.ch[ch].scfsi[1] = 0) or (gr = 0)) then begin
    ...
    if ((FSI.ch[ch].scfsi[2] = 0) or (gr = 0)) then begin
    ...
    if ((FSI.ch[ch].scfsi[3] = 0) or (gr = 0)) then begin
...
var ch, gr: Cardinal;
begin
  if (FHeader.Version = MPEG1) then begin
    ...
    for gr := 0 to 1 do begin
      for ch := 0 to FChannels-1 do begin
        ...
        if (FSI.ch[ch].gr[gr].window_switching_flag <> 0) then begin
          ...
          if (FSI.ch[ch].gr[gr].block_type = 0) then begin
            // Side info bad: block_type == 0 in split block
            Result := False;
            exit;
          end else if (FSI.ch[ch].gr[gr].block_type = 2) and (FSI.ch[ch].gr[gr].mixed_block_flag = 0) then
    ...
    for ch := 0 to FChannels-1 do begin
      ...
      if (FSI.ch[ch].gr[0].window_switching_flag <> 0) then begin
        ...
        if (FSI.ch[ch].gr[0].block_type = 0) then begin
          ...
          Result := False;
          exit;
        end else if (FSI.ch[ch].gr[0].block_type = 2) and (FSI.ch[ch].gr[0].mixed_block_flag = 0) then
...
begin
  ...
  if ((FSI.ch[ch].gr[gr].window_switching_flag <> 0) and (FSI.ch[ch].gr[gr].block_type = 2)) then begin
    // Region2.
  ...
  // Read bigvalues area
  i := 0;
  while (i < (FSI.ch[ch].gr[gr].big_values shl 1)) do begin
...
begin
  if (IsPos = 0) then begin
    FK[0, i] := 1.0;
    FK[1, i] := 1.0;
  end else if (IsPos and 1 <> 0) then begin
